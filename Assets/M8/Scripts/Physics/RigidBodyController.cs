...
public class RigidBodyController : MonoBehaviour {
    ...
    void OnCollisionStay(Collision col) {
        ...
        foreach(ContactPoint contact in col.contacts) {
            ...
            if(sideBounceImpulse > 0.0f && colFlag == CollisionFlags.Sides)
                GetComponent<Rigidbody>().AddForce(contact.normal * sideBounceImpulse, ForceMode.Impulse);
    ...
    protected virtual void Awake() {
        ...
        if(GetComponent<Collider>() != null) {
            if(GetComponent<Collider>() is SphereCollider)
                ...
                mRadius = ((CapsuleCollider)GetComponent<Collider>()).radius;
        }
    }
    ...
    protected virtual void FixedUpdate() {
        ...
        if(mCurMoveAxis != Vector2.zero) {
            ...
            GetComponent<Rigidbody>().drag = isUnderWater ? waterDrag : isGrounded ? groundDrag : airDrag;
        }
    }

...
class tk2dSpriteEditor : Editor
{
	...
	protected void DrawSpriteEditorGUI()
	{
        ...
        if (targetSprites[0].Collection != null)
        {
			...
			int newRenderLayer = EditorGUILayout.IntField("Render Layer", targetSprites[0].RenderLayer);
			if (newRenderLayer != targetSprites[0].RenderLayer) {
            	Undo.RegisterUndo(targetSprites, "Render Layer");
            	foreach (tk2dBaseSprite s in targetSprites) {
            		s.RenderLayer = newRenderLayer;
            	}
			}
